...
**Verified conventions (confirmed on Wulver; build to exactly these):** `ar.subgraph_isomorphism` is **induced** (NOT `subgraph_monomorphism`); host + pattern loaded **symmetrized** (each undirected edge both directions); returns `(induced copies) × |Aut(H)|`; the call returns **two** arrays: `result[0]` = `isoArr`, flat host-vertex embeddings of length `num_embeddings × n_pattern`, reshape to `[num_embeddings, n_pattern]`; `result[1]` = `isoMapper`, the pattern-vertex order (`nodeMapGraphG2`) repeated once per embedding.

**The `reorder_type` knob is decisive (resolved 2026-06-16).** The feature-extraction (correctness) path uses **`reorder_type="None"`**: the Chapel server (`SubgraphSearch.chpl` ~line 1060 branches on `reorderType != "None"`; the Python docstring never documented `"None"`) does **no** pattern-vertex reordering, so embeddings come back in our **ORIGINAL pattern-vertex order — column j == pattern vertex j**, and `isoMapper` is the identity. Confirmed on Wulver: C5 wedge embeddings put the center in the MIDDLE column (pos 1), and Cora induced wedges match ORCA (0 adjacent / 94822 non-adjacent). The backend (`hipermotif_backend.py`) calls with `reorder_type="None"` by default (`DEFAULT_REORDER_TYPE`), still reads `result[1]`, and **asserts it is the identity** (`_extract_mapper(require_identity=True)`) as a guard against reordering being silently reintroduced. It also runs `hipermotif_patterns.assert_induced_embeddings` per pattern — a regression guard that every pattern non-edge maps to a host non-edge (catches monomorphism/adjacency bugs at the source).

**Bug history (2026-06-15/16, fixed):** the first cluster gate run failed `not divisible by |Aut|` on `p3`/`paw`/`p4` (multi-orbit patterns) on Shrikhande/Cora while K4/C5/triangle/4-clique (single-orbit or symmetric) passed and the local mock passed. Cause: the default **`reorder_type="structural"` permutes the pattern's vertices**, so output column j held original pattern vertex `isoMapper[j]`, NOT j; the backend ignored `result[1]` and the mock used identity column order, so multi-orbit patterns mixed orbit tallies (single-orbit/symmetric ones are invariant to the permutation → passed). The earlier "Cora 100% adjacent" symptom was a broken adjacency check reading center-vs-endpoint because the reorder had moved the center to column 0. Root cause found by reading Arachne `SubgraphSearch.chpl` runSearch (~1882–1893, isoMapper construction) + `SubgraphSearchMsg.chpl` (77–87, two-array "vertices" return). **Resolution: use `reorder_type="None"`** (removes the permutation at the source). The `structural`+mapper-inversion path is retained — `normalize_embeddings(pattern, emb, n, mapper)` inverts the permutation, and `induced_embeddings(..., reorder=True)` + `test_remap_required_for_multiorbit_patterns` keep it under test — for the **timing experiments** below.

**Reorder note for SCALING/TIMING (record; do not let timing use the slow path by accident):** `reorder_type="None"` may be **slower** because structural reordering optimizes the search. We use `"None"` for one-time, correctness-critical feature extraction. For the eventual Wulver scaling/timing experiments, run with reordering **ON** (`reorder_type="structural"`, with the mapper inversion applied) so HiPerMotif's performance is not understated — these are two different needs.
