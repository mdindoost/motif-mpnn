module SubgraphSearch {
  ...
  */
  ...
  proc runSearch(g1: SegGraph, g2: SegGraph, returnIsosAs:string,
    ...
    // ==========================================================================
    // FIX (parallel iso-block interleaving). The result-collecting foralls below
    // (edgeCentricStateInjection 1712, vertexCentricStateInjection 1754, and the
    // recursive savers 1546/1600) each merge a child's matches into a SHARED list
    // with `for m in blk do dst.pushBack(m)`. `parSafe=true` makes each single
    // pushBack atomic, but NOT the whole block, so under maxTaskPar>1 the
    // numSubgraphVertices-element blocks of different tasks INTERLEAVE: the total
    // element count stays exact (every element pushed once) but the per-match
    // grouping is scrambled (reshape(-1,k) then yields mixed/repeated rows).
    // appendBlock() serializes ONLY the append so each block lands contiguously;
    // the search itself stays fully parallel. (A lower-contention alternative is
    // per-task local lists concatenated after each forall; this lock keeps the
    // change minimal and obviously correct.)
    // ==========================================================================
    var appendLock$: sync bool;            // sync var used as a mutex; starts EMPTY = unlocked
    proc appendBlock(ref dst, const ref blk) {
      appendLock$.writeEF(true);           // acquire: blocks until EMPTY, then sets FULL
      defer appendLock$.readFE();          // release on scope exit (exception-safe): FULL->EMPTY
      for m in blk do dst.pushBack(m);     // whole block appended as one contiguous unit
    }
    ...
    proc recursiveMatchSaverFast(state: owned State, depth: int): list(int,parSafe=true) throws {
      ...
      forall (n1, n2) in candidatePairs with (ref state, ref allmappings) {
        ...
        if (findingIsos && isFeasible_ISO(n1,n2,state))||(findingMonos && isFeasible_MONO(n1,n2,state)) {
          ...
          appendBlock(allmappings, newMappings);
        }
      }
      return allmappings;
    }
        ...
        if vertexFlagger[srcNodesG1[edgeIndex]] && srcNodesG1[edgeIndex] != dstNodesG1[edgeIndex] {
          ...
          if edgeChecked {
            ...
            if countOnly && (limitSize || limitTime || printProgressCheck) {
              ...
              appendBlock(solutions, newMappings);
            } else {
