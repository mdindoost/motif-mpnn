...
vertex's count for a given orbit is similarly divided by the orbit stabilizer size
$|\mathrm{Aut}(H)|/|\text{orbit}|$ so that each embedded copy contributes once per
...
\For{each pattern $H \in \mathcal{H}$}
  ...
  \For{each orbit $O_j$ with stabilizer $b_j=|\mathrm{Aut}(H)|/|O_j|$}
    \For{each pattern position $p \in O_j$}
